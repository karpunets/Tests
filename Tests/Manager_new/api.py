...
class TestAuthorizationServer:
    ...
    def test_get_token_for_root(self):
        ...
        response = req.post(TestAuthorizationServer.url, data)
        print(response.json())
    ...
    def test_get_token_for_system_roles(self, add_user_with_role, role_name):
        ...
        data = parse_request("auth", {"$principal": user['login'],
                                      "$credential": "qwerty"})
        response = req.post(TestAuthorizationServer.url, data['request'])
    ...
    def test_signin_with_token_check_login(self, add_user_with_role):
        ...
        response_auth = req.post(TestAuthorizationServer.url, data['request'])
        response_auth = response_auth.json()
        headers = {'content-type': "application/json;charset=UTF-8"}
        headers[response_auth['name']] = response_auth['token']
        response_current_account = req.get('account', headers=headers)
...
class TestGroups:
    ...
    def test_get_groups(self):
        response = req.get(TestGroups.url)
    ...
    def test_get_group_by_id(self, group):
        response = req.get(TestGroups.url, id_to_url=group['groupId'])
        print(response.json())
    ...
    def test_get_group_with_unknownId(self):
        ...
        response = req.get(TestGroups.url, id_to_url=unknown_id)
        excepted_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: GROUP by groupId=%s not found' % unknown_id}
    ...
    def test_edit_group(self, group):
        ...
        response = req.put(TestGroups.url, data=data['request'], id_to_url=group['groupId'])
    ...
    def test_edit_group_with_unknown_groupId(self, group):
        ...
        data = parse_request("put_group", {"$name": random_string(),
                                           "$groupId": group['groupId']})
        response = req.put(TestGroups.url, data=data['request'], id_to_url=unknown_group_id)
        expected_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: GROUP by groupId=%s not found' % unknown_group_id}
        assert (response.status_code, response.json()) == (400, expected_response)

    @allure.feature('Функциональный тест')
    @allure.story('Удаляем группу')
    def test_delete_group(self, group):
        response = req.delete(TestGroups.url, id_to_url=group['groupId'])
        assert (response.status_code, response.json()) == (200, group)
    ...
    def test_delete_group_with_unknown_id(self):
        unknown_group_id = random_string()
        response = req.delete(TestGroups.url, id_to_url=unknown_group_id)
        expected_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: GROUP by groupId=%s not found' % unknown_group_id}
        assert (response.status_code, response.json()) == (400, expected_response)
    ...
    def test_delete_group_with_child(self, immutable_group_with_child):
        ...
        response = req.delete(TestGroups.url, id_to_url=group_id)
        print(response.json())
        expected_response = {
...
class TestRoles:
    ...
    def test_get_roles(self, role):
        response = req.get(TestRoles.url)
    ...
    def test_get_role_by_id(self, role):
        response = req.get(TestRoles.url, id_to_url=role['roleId'])
        role.pop("templateRole")
    ...
    def test_get_role_by_unknown_id(self):
        ...
        response = req.get(TestRoles.url, id_to_url=unknown_role_id)
        expected_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: ROLE by roleId=%s not found' % unknown_role_id}
        assert (response.status_code, response.json()) == (400, expected_response)
    ...
    def test_delete_role(self, role):
        role.pop("templateRole")
        response = req.delete(TestRoles.url, id_to_url=role['roleId'])
        assert (response.status_code, response.json()) == (200, role)
    ...
    def test_delete_role_by_unknown_id(self):
        unknown_role_id = random_string()
        response = req.delete(TestRoles.url, id_to_url=unknown_role_id)
        expected_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: ROLE by roleId=%s not found' % unknown_role_id}
        assert (response.status_code, response.json()) == (400, expected_response)
    ...
    def test_edit_role(self, role, immutable_group_with_child):
        ...
        response = req.put(TestRoles.url, data['request'], id_to_url=role['roleId'])
...
class TestUsers:
    ...
    def test_add_user_with_required_fields(self, clear_data, immutable_role, userGroupRoles):
        ...
        response = req.post(TestUsers.url, data['request'])
        print(response.json())
    ...
    def test_add_user_with_existing_agentId(self, immutable_role, userGroupRoles, immutable_user):
        ...
        print(response.json())
        expected_response = {
            'COMMON_ENTITY_WITH_SUCH_FIELD_EXISTS': 'CommonEntityWithSuchFieldExists: AGENT ID =%s already exists' % existing_agent_id}
        assert (response.status_code, response.json()) == (409, expected_response)
    ...
    def test_get_user(self, immutable_user):
        response = req.get(TestUsers.url, id_to_url=immutable_user['userId'])
    ...
    def test_get_user_with_unknown_id(self):
        ...
        response = req.get(TestUsers.url, id_to_url=unknown_user_id)
        expected_response = {
    ...
    def test_get_deleted_user(self, immutable_deleted_user):
        response = req.get(TestUsers.url, id_to_url=immutable_deleted_user['userId'])
    ...
    def test_delete_user(self, user):
        response = req.delete(TestUsers.url, id_to_url=user['userId'])
    ...
    def test_delete_user_by_unknown_userId(self):
        unknown_user_id = random_string()
        response = req.delete(TestUsers.url, id_to_url=unknown_user_id)
        expected_response = {
            'COMMON_REQUESTED_RESOURCES_NOT_FOUND': 'CommonRequestedResourcesNotFound: USER not found by userId=%s' % unknown_user_id}
        assert (response.json(), response.status_code) == (expected_response, 400)
    ...
    def test_get_user_enabled_false(self, add_user_with_role):
        ...
        response = req.get(TestUsers.url, id_to_url=disabled_user['userId'])
    ...
    def test_delete_user_enabled_false(self, add_user_with_role):
        disabled_user = add_user_with_role(enabled=False)
        response = req.delete(TestUsers.url, id_to_url=disabled_user['userId'])
        assert (response.json(), response.status_code) == (disabled_user, 200)
